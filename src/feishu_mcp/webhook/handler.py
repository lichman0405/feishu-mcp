"""
webhook/handler.py — Feishu Event Subscription Handler

Provides a FastAPI application to receive events pushed by Feishu:
- im.message.receive_v1: @bot messages (triggers Agent processing)
- drive.file.permission_member_added_v1: cloud document permission requests

Startup:
  uvicorn feishu_mcp.webhook.handler:app --host 0.0.0.0 --port 8080

Local debugging (get a public URL):
  ngrok http 8080
  → Paste the ngrok URL into the Feishu developer console under "Event Subscription" callback address
"""
...
async def _handle_message_receive(payload: dict):
    ...
    # Connect to Agent message queue or trigger tool calls directly
